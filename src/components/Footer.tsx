// components/Footer.tsx - VERSION WORDPRESS AVEC BULLES INTERACTIVES
...
function useFooterData() {
  ...
  useEffect(() => {
    // ⚠️ CHANGEZ CETTE URL SELON VOTRE CONFIG
...
export function Footer() {
  ...
  // Error state - Valeurs par défaut
  if (error || !footerData) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          ...
          >
            ...
            <div className="space-y-3">
              <a href={`tel:${contact.phone.replace(/\s/g, '')}`} className="flex items-center gap-3 text-gray-400 hover:text-white transition-colors group">
                <div className="p-2 rounded-lg group-hover:bg-white/10 transition-colors" style={{ backgroundColor: `${colors.primary}10` }}>
                  <Phone className="size-4" style={{ color: colors.primary }} />
                </div>
                <span className="text-sm">{contact.phone}</span>
              </a>
              <a href={`mailto:${contact.email}`} className="flex items-center gap-3 text-gray-400 hover:text-white transition-colors group">
                <div className="p-2 rounded-lg group-hover:bg-white/10 transition-colors" style={{ backgroundColor: `${colors.secondary}10` }}>
                  <Mail className="size-4" style={{ color: colors.secondary }} />
                </div>
            ...
            <ul className="space-y-3">
              {quickLinks.map((link, index) => (
                ...
                >
                  ...
                  >
                    <ChevronRight className="size-4 group-hover:translate-x-1 transition-transform" style={{ color: colors.primary }} />
              ...
              {activities.map((activity, index) => (
                ...
                >
                  ...
                  >
                    ...
                    <span className="group-hover:translate-x-1 transition-transform">{activity.name}</span>
                  </Link>
                </motion.li>
              ))}
            </ul>
...
function ParkBubble({ park, index, primaryColor, secondaryColor, backgroundColor }: ParkBubbleProps) {
  ...
  const positions = [
    { top: '5%', left: '8%', size: 70 },       // Parc 1
    { top: '10%', left: '62%', size: 68 },     // Parc 2
    { top: '52%', left: '15%', size: 65 },     // Parc 3
    { top: '55%', left: '68%', size: 62 },     // Parc 4
    { top: '28%', left: '38%', size: 72 },     // Parc 5 (centre)
  ];

  const position = positions[index] || positions[0];
  const parkName = park.location.split(',')[0];
